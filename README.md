...
管理ホスト設定は、ファイル名と一致する単一の `Host`、`HostName`、管理対象の
`IdentityFile`、`IdentitiesOnly yes` を必須とします。`list` および各ホスト操作はこの
契約を満たさない設定を拒否し、`audit` は不整合として報告します。

`generate` は既存ファイルを上書きせず、失敗時には処理が作成したステージングファイルを
除去するため、外部ファイルとの競合がなければ再実行できます。`remove` は設定と鍵をすべて
検査してから鍵を削除し、最後に設定を削除します。鍵削除の失敗時には設定が再試行情報として
残ります。

`set` は契約を満たす設定の接続先、ユーザー、ポートのみを更新します。構造が壊れた設定は
`set` や `remove` の対象にならないため、`audit` の報告に従ってバックアップから正規の設定を
復元してから操作します。

リモートマシンへの接続手順は
[Remote machine login with ssv](docs/remote-login.md) を参照してください。
